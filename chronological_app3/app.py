...
# handle form data from hopepage
@app.route('/get-timeline-data-horizontal', methods=['POST'])
def handle_timeline_data_horizontal():
    #rows = connect("SELECT audioLink FROM chronological_view;")
    rows = connect("SELECT * FROM chronological_view;")
    return render_template('timeline_h.html',rows=rows)    
...
def handle_data():
    #retrieves data based on hardcoded SQL query 
    #data = (request.form['file_name_box'])
    data = (request.form)
    ...
    return render_template('timeline_new.html',rows=rows)     
